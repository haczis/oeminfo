<div id="featured">	
	...
	if (get_option('professional_use_pages') == 'false') query_posts("showposts=$featured_num&cat=".get_catId($featured_cat));
		...
		query_posts(array('post_type' => 'page',
						'orderby' => 'menu_order',
						'order' => 'ASC',
						'post__in' => get_option('professional_feat_pages'),
		...
		<?php for ($i = 1; $i <= $featured_num; $i++) { ?>
			<div class="slide">
				...
				<div class="description">
					...
					<a href="<?php echo($arr[$i]["permalink"]); ?>" class="readmore"><span><?php _e('read more','Professional'); ?></span></a>
				</div> <!-- end .description -->	
			</div> <!-- end .slide -->
		<?php }; ?>
	</div> <!-- end #slides-->
		...
		<div id="controllers-main">
			<?php for ($i = 1; $i <= $featured_num; $i++) { ?>
				<a href="#"<?php if($i == 1) echo(' class="active"'); if($i == $featured_num) echo(' class="last"'); ?> rel="<?php echo($i); ?>">
					...
					<span class="tooltip">
						<span class="heading"><?php echo($arr[$i]["fulltitle"]); ?></span>
	...
	<a href="#" id="left-arrow"><?php _e('Previous','Professional'); ?></a>
	<a href="#" id="right-arrow"><?php _e('Next','Professional'); ?></a>
